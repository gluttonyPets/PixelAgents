            ...
            for (var mi = 0; mi < mainModules.Count; mi++)
            {
                ...
                try
                {
                    ...
                    else if (pm.AiModule.ModuleType == "VideoEdit")
                    {
                        ...
                        if (requireConfirm)
                        {
                            // Build a preview of what will be sent to Json2Video
                            var previewPayload = new Dictionary<string, object>
                            {
                                ["script"] = editInput,
                                ["videoUrls"] = videoUrls,
                                ["config"] = config.Where(kv => kv.Key != "videoUrls" && kv.Key != "videoPrompt" && kv.Key != "requireConfirmation")
                                    .ToDictionary(kv => kv.Key, kv => kv.Value),
                            };
                            var previewJson = JsonSerializer.Serialize(previewPayload, new JsonSerializerOptions { WriteIndented = true });
                            ...
                            stepExecution.Status = "WaitingForVideoEditConfirm";
                            stepExecution.InputData = previewJson;
                            execution.PausedAtStepOrder = pm.StepOrder;
                            execution.PausedStepData = JsonSerializer.Serialize(pauseState);
                            execution.Status = "WaitingForVideoEditConfirm";
                            await db.SaveChangesAsync();

                            await _logger.LogAsync(projectId, executionId, "info",
                                $"VideoEdit pausado: esperando confirmacion del usuario antes de enviar a Json2Video",
                                pm.StepOrder, stepName);
                            await _logger.LogStepProgressAsync(projectId, pm.Id, "WaitingForVideoEditConfirm");
...
{executionContext}";
        ...
        public async Task<ProjectExecution> ResumeFromVideoEditConfirmAsync(
            Guid executionId, bool approved, UserDbContext db, string tenantDbName, CancellationToken ct = default)
        {
            ...
            if (execution.Status != "WaitingForVideoEditConfirm")
                throw new InvalidOperationException($"La ejecucion no esta esperando confirmacion de VideoEdit (status={execution.Status})");

            var project = await db.Projects
                .Include(p => p.ProjectModules).ThenInclude(pm => pm.AiModule).ThenInclude(m => m!.ApiKey)
                .Include(p => p.ProjectModules).ThenInclude(pm => pm.InputMapping)
            ...
            if (!approved)
            {
                // User cancelled: mark step and execution as cancelled
                var cancelledStepExec = execution.StepExecutions.FirstOrDefault(s => s.StepOrder == pausedStep);
                if (cancelledStepExec is not null)
                {
                    cancelledStepExec.Status = "Cancelled";
                    cancelledStepExec.CompletedAt = DateTime.UtcNow;
                }
                execution.PausedAtStepOrder = null;
                execution.PausedStepData = null;
                execution.Status = "Cancelled";
                ...
                await _logger.LogAsync(project.Id, execution.Id, "warning",
                    "VideoEdit: el usuario cancelo la ejecucion");
                return execution;
            }

            // User confirmed: restore state and continue execution from the paused VideoEdit step
            ...
            // Clear pause state and set running
            execution.PausedAtStepOrder = null;
            execution.PausedStepData = null;
            execution.Status = "Running";
            await db.SaveChangesAsync();
            ...
            await _logger.LogAsync(project.Id, execution.Id, "info",
                "VideoEdit: usuario confirmo, reanudando ejecucion...");

            // Re-execute from the paused step onwards (inclusive — the VideoEdit step itself)
            ...
            for (var mi = 0; mi < mainModules.Count; mi++)
            {
                ...
                // For the paused VideoEdit step, reuse the existing StepExecution; for later steps, create new
                StepExecution stepExecution;
                if (pm.StepOrder == pausedStep)
                {
                    stepExecution = execution.StepExecutions.First(s => s.StepOrder == pausedStep);
                ...
                try
                {
                    ...
                    // Skip requireConfirmation on resume — user already confirmed
                    ...
                    {
                        // Re-resolve the edit input and video URLs same as main loop
                        var editInput = "";
                        if (config.TryGetValue("videoPrompt", out var vp2))
                            editInput = vp2 is JsonElement vp2El ? vp2El.GetString() ?? "" : vp2?.ToString() ?? "";
                        if (string.IsNullOrWhiteSpace(editInput))
                            editInput = inputs.Count > 0 ? inputs[0] : "";
                        if (string.IsNullOrWhiteSpace(editInput))
                        {
                            foreach (var prevOrder in stepOutputs.Keys.Where(k => k < pm.StepOrder).OrderByDescending(k => k))
                            {
                                if (stepModuleTypes.TryGetValue(prevOrder, out var pt2) && (pt2 == "VideoSearch" || pt2 == "Video"))
                                    continue;
                                if (stepOutputs.TryGetValue(prevOrder, out var po2))
                                {
                                    if (po2.Items.Count > 0) { editInput = string.Join("\n\n", po2.Items.Select(i => i.Content)); break; }
                                    if (!string.IsNullOrWhiteSpace(po2.Content)) { editInput = po2.Content; break; }
                                }
                            }
                        }
                        ...
                        var editContext = new AiExecutionContext
                        {
                            ModuleType = pm.AiModule.ModuleType, ModelName = pm.AiModule.ModelName,
                            ApiKey = apiKey, Input = editInput,
                            ProjectContext = project.Context, Configuration = config,
                        };
                        ...
                        if (result.FileOutput is not null)
                        {
                            ...
                            var execFile = new ExecutionFile
                            {
                                ...
                                FileName = fileName, ContentType = result.ContentType ?? "video/mp4",
                                FilePath = Path.Combine($"step_{pm.StepOrder}", fileName),
                    ...
                    {
                        // For non-VideoEdit steps after the paused one, use generic execution
                        var ctx = new AiExecutionContext
                        {
                            ModuleType = pm.AiModule.ModuleType, ModelName = pm.AiModule.ModelName,
                            ApiKey = apiKey, Input = inputs.Count > 0 ? inputs[0] : "",
                            ProjectContext = project.Context, Configuration = config,
                        };
                        var result = await provider.ExecuteAsync(ctx);
                        stepResults[pm.StepOrder] = result;
                        stepModuleTypes[pm.StepOrder] = pm.AiModule.ModuleType;
                        stepExecution.EstimatedCost += result.EstimatedCost;
