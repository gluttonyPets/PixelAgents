        ...
        IReadOnlyCollection<string>? suppressedKeys = null)
    {
        ...
        if (string.Equals(moduleType, "Image", StringComparison.OrdinalIgnoreCase) && sceneCount > 1)
        {
            rules.Add(new ActiveRule
            {
                ...
                Body = $"Este modulo genera {sceneCount} imagenes con {sceneCount} llamadas independientes, no con una llamada de n={sceneCount} (para el proveedor, n son copias del mismo prompt y saldrian todas iguales).\n\nPara repartir busca en el texto de entrada las marcas ===IMAGEN 1===, ===IMAGEN 2===, ... : lo que hay antes de la primera marca se manda como contexto comun a todas las imagenes y cada bloque posterior se manda como prompt de su imagen.\n\nSi la conexion de entrada declara un contrato JSON (donde las marcas romperian el JSON), el reparto se hace por la lista del contrato: cada elemento de la lista es el prompt de una imagen y las claves sueltas que la acompanan son contexto comun.\n\nLas marcas o los elementos los escribe el modulo de texto conectado a la entrada, al que se le inyecta automaticamente la instruccion de planificar {sceneCount} prompts. Si el texto llega sin separar no hay reparto posible: se avisa en el log de la ejecucion y las {sceneCount} imagenes salen del mismo prompt.",
            });
        }
