...
{
    public static List<PortDefinition> GetPorts(string moduleType, string providerType = "", int sceneCount = 0)
    {
        ...
        switch (moduleType)
        {
            ...
            case "VideoEdit":
                ...
                // Dynamic scene inputs
                var scenes = Math.Max(sceneCount, 1);
                for (int i = 1; i <= scenes; i++)
                {
                    ports.Add(new($"input_scene_{i}_video", $"Escena {i} - Video", PortDataType.Video, isInput: true, isRequired: true));
                    ports.Add(new($"input_scene_{i}_script", $"Escena {i} - Guion", PortDataType.Text, isInput: true));
                }
